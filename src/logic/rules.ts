import { Place, Slots, Geo, Intent, ItinerarySlots } from '../types.js';
...
export function recommend(intent: Intent, slots: Slots, origin?: Geo): Place[] {
  ...
  const tagMap: { [key in Intent]?: string } = {
    nuong: 'nuong',
    cafe: 'cafe',
    sight: 'sight',
    food: 'food',
...
  
  ...
  if (slots.theme) {
    ...
    // Lọc cho intent 'sight' - THAM QUAN
        ...
        results = results.filter(p => 
          (p.tags.includes('history') || p.tags.includes('landmark') || p.tags.includes('sight')) &&
          !p.tags.includes('food') && 
          !p.tags.includes('nuong') && 
          !p.tags.includes('lau')
        );
...
      if (slots.area === 'gần chợ') return dist <= 2.5;
      if (slots.area === 'ngoại thành') return dist >= 8;
...
  let results: Place[] = allPlaces.filter(p => 
    p.tags.includes('sight') || 
    p.tags.includes('cafe') || 
    p.tags.includes('history') ||
    p.tags.includes('nature') ||
    p.tags.includes('check-in')
  );
  let foodPlaces: Place[] = allPlaces.filter(p => p.tags.includes('food') && !p.tags.includes('cafe'));
  
  // 1. Lọc theo CHỦ ĐỀ (Theme)
  const theme = slots.theme ? normalizeText(slots.theme) : '';
  let themeFilteredResults: Place[] = [];
