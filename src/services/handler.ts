import { SessionContext, Geo, Place, BotResponse, ClientPayload, ItinerarySlots } from '../types.js';
...
export async function handleMessage(
  ...
  if (payload && payload.action === 'go_node') {
    ...
    const node = FLOW.nodes[nodeName as keyof typeof FLOW.nodes];
  ...
  if (q) {
    ...
    case 'transport':
      ...
      case 'itinerary':
        ...
        if (missingSlots.length > 0) {
          ...
          return {
              ...
              choices: [
                ...finalChoices,
                { label: '📥 Xuất file TXT', payload: { action: 'export_itinerary', value: 'txt' } as any }
              ] as any 
          };
        }
        break;
      ...
      default: // 'unknown'
        if (ctx.lastIntent === 'itinerary' && ctx.itinerarySlots) {
          const requiredSlotsDefault: (keyof ItinerarySlots)[] = [
            'days', 'groupSize', 'traveler_type', 'budget', 'theme', 'pace'
          ];
